...
- `Concurrency` is a general concept that involves managing multiple tasks at once, but not necessarily simultaneously (in parallel) since it can switch between tasks. It encompasses multiple paradigms, such as multithreading, multiprocessing, event-driven programming, and asynchronous programming.
  ...
  - `Multithreading` is a specific form of concurrency where multiple threads run within a single process to perform multiple tasks in parallel, `sharing the same memory space`. It is suitable for `data I/O-bound operations`.
  - `Multiprocessing`: is a specific form of concurrency where multiple processes run `independently and simultaneously`, `each with its own memory space`, potentially using multiple CPUs to achieve parallelism. It allows `true parallelism`, making it suitable for `CPU-bound operations`.