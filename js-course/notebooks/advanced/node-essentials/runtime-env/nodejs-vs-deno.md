...
A `runtime environment` is required to run `asynchronous (non-blocking) operations` and to run JavaScript code outside the browser on the server/backend side.

Node.js is a [cross-platform, open-source JavaScript runtime environment](https://en.wikipedia.org/wiki/Node.js) (not a framework or programming language) written in C++ that uses Google's V8 JavaScript engine to execute code outside the browser.

It traditionally uses an event-driven model with a single-threaded event loop for handling asynchronous (non-blocking) I/O-bound operations.

However, the [worker_threads module](../core-modules/worker-threads.md) of Node.js can be used to execute CPU-bound operations, such as in-browser AI inference, without blocking the main thread.
...
The Node.js runtime environment of consists of a `Node API`, an `Event Queue`, an `Event Loop`, and `Google's V8 Engine`.
...
- `Google's V8 JavaScript Engine`: is the JS engine that parses JS source code into optimized machine code (binary code) with Just-In-Time (JIT) compilation at runtime.
  ...
  - Memory Heap: is a fixed-size region in memory used for dynamic memory allocation, i.e., to allocate memory for storing, without order, dynamic objects (hash tables, arrays, functions, etc.) created at runtime since they do not have a fixed size/length (unknown). It enables efficient storage and retrieval of data during a program's execution. Memory management is handled by the `garbage collector`.

- `Event Queue`: holds callback functions from asynchronous operations that are ready to be executed by the engine's Call Stack. Order of execution: `process.nextTick` queue (highest priority), `microtask` queue (high priority), and `macrotask` queue (standard priority).

- `Event Loop`: it continuously checks if the `Call Stack` is empty. If it is, and there are tasks in the `Event Queue`, the Event Loop takes the first task from the Event Queue and pushes it onto the Call Stack for execution.
  - The Event Loop implementation is provided by the [libuv library](https://docs.libuv.org/en/v1.x/loop.html). This library is focused on [asynchronous I/O-bound operations](https://docs.libuv.org/en/v1.x/), and the event loop is tied to a [single thread](https://docs.libuv.org/en/v1.x/design.html). This means that Node.js executes multiple tasks on a single event loop main thread by default.
  - However, libuv has the `threads pool feature`, which is an implementation mechanism that uses multiple threads to run blocking operations in the background so that the main event loop thread is not blocked. This is not the same as Node.js's [worker threads module](https://nodejs.org/api/worker_threads.html).
  - The JS execution is single-threaded, but the runtime/process is multi-threaded. The Node.js event loop has six phases, which are:
    - Timer Phase.
    - Pending Callbacks Phase.
    - Idle Phase.
    - Poll Phase.
...
- Note: Rust is a programming language designed at Mozilla that has performance comparable to that of C. While C programs are prone to human errors that can cause buffer overflows and dangling pointers, Rust includes safety guarantees around memory and concurrency (e.g., multithreading) with a small overhead.

## Deno Features

- Modules: Unlike Node.js, which uses CommonJS syntax, Deno uses ECMAScript (ESM) syntax by default. Modules can be imported using a URL with the import keyword instead of the require keyword.
